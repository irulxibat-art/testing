...
def add_note(user_id, market, open_p, tp, sl, lot, side, pl_per_lot, pl_total, note):
    ts = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") if False else datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with get_conn() as conn:
        cur = conn.cursor()
        cur.execute("""
...
with col2:
    ...
    try:
        df_plot = df.copy()
        df_plot['Total P/L'] = df_plot['Total P/L'].astype(str).str.replace(",","")
        df_plot['Total P/L'] = df_plot['Total P/L'].astype(float)

        fig1, ax1 = plt.subplots()
        ax1.plot(df_plot['Timestamp'], df_plot['Total P/L'])
        ...
        ax2.plot(df_plot['Timestamp'], df_plot['Equity'])
