#!/usr/bin/env python3
"""
Simple Streamlit port of trading_app.py
...
def add_note(user_id, market, open_p, tp, sl, lot, side, pl, note):
    ts = datetime.datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") if False else datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with get_conn() as conn:
        cur = conn.cursor()
        cur.execute("""
...
with col2:
    ...
    if d1 == datetime.date.today().isoformat() and d2 == datetime.date.today().isoformat():
        # If user didn't pick anything, date_input returns today's date by default in some streamlit versions.
        d1 = d2 = None
