...
#!/usr/bin/env python3
"""
Mobile-friendly Streamlit trading app (full features):
...
- Mobile-first layout (single column), larger inputs/buttons (CSS)
- Plots: P/L per trade + equity curve (matplotlib)
Notes:
- Ensure requirements include: streamlit>=1.30, pandas, matplotlib, websocket-client
"""
import os
import sqlite3
import hashlib
...
# websocket-client (ensure this is in requirements)
...
st.markdown("# Trading App (Mobile)")
st.caption("Ringkas & ramah layar HP — semua fitur tetap ada")

# --- top area with account & ws controls inside an expander ---
with st.expander("Akun, Cache & Live Price", expanded=True):
    ...
    with c2:
        if st.button("Unsubscribe"):
            ...
            st.rerun()
    if st.button("Stop streamer"):
        streamer.stop()
    active = streamer.list_symbols()
    if active:
        st.write("Active subs:")
        for s in active:
            p = streamer.get_price(s)
            st.write(f"- {s}: {p[0]:,.8f} (ts {p[1]})" if p else f"- {s}: no price yet")
    else:
...
if st.session_state['user_id'] is None:
    st.info("Silakan login atau registrasi lewat menu di atas.")
    st.stop()

# Main single-column mobile UI
...
market = st.selectbox(
    "Market",
    ["XAUUSD", "BTCUSD", "USTEC", "EURUSD", "USDJPY"],
    index=(["XAUUSD", "BTCUSD", "USTEC", "EURUSD", "USDJPY"].index(pre_market)
           if pre_market in ["XAUUSD", "BTCUSD", "USTEC", "EURUSD", "USDJPY"]
           else 0)
open_p_str = st.text_input("Open (kosong = pakai live)", value=str(pre_open))
...
# Buttons group (stacked nicely for mobile)
...
if st.button("Hapus") and edit_choice:
    ...
    try:
        ...
        def to_float_safe(val):
            try:
                ...
                return 0.0
        df_plot['Total P/L'] = df_plot['Total P/L'].apply(to_float_safe)

        # ensure Timestamp sorted (as strings assume ISO format)
        try:
            df_plot['Timestamp_sort'] = pd.to_datetime(df_plot['Timestamp'])
            df_plot = df_plot.sort_values('Timestamp_sort')
        except Exception:
            pass

        fig1, ax1 = plt.subplots()
        ax1.plot(df_plot['Timestamp'], df_plot['Total P/L'], marker='o')
        ax1.set_title("Total P/L per Trade")
        ax1.set_xlabel("Waktu")
        ax1.set_ylabel("Total P/L")
        ax1.tick_params(axis="x", rotation=45)
        st.pyplot(fig1)

        df_plot["Equity"] = df_plot["Total P/L"].cumsum()
        fig2, ax2 = plt.subplots()
        ax2.plot(df_plot['Timestamp'], df_plot['Equity'], marker='o')
        ax2.set_title("Equity Curve (Akumulasi P/L)")
        ax2.set_xlabel("Waktu")
        ax2.set_ylabel("Akumulasi P/L")
